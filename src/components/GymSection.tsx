...
const GymSection = () => {
  ...
  useEffect(() => {
    ...
    if (savedDotStates) {
      ...
      const consistencyRate = (totalGymDays / 30) * 100;
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-2 mb-6">
        <Dumbbell className="h-6 w-6 text-gym" />
        <h2 className="text-2xl font-bold">Gym Tracking</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-lg">Gym Summary</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground">Total Gym Days</p>
                <p className="text-2xl font-bold text-gym">{gymStats.totalGymDays} days</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Consistency Rate</p>
                <p className="text-2xl font-bold">{Math.round(gymStats.consistencyRate)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {Object.entries(gymStats.weekdayDistribution).map(([day, count]) => (
                  <TableRow key={day}>
